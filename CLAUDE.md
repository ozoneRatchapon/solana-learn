...
## เข้ามาใหม่ ให้รันตัวนี้ก่อนอย่างอื่น

```bash
./scripts/status.sh
```

ตอบทีเดียวครบ: จำนวนของทุกชนิด · ตัวตรวจตัวไหนผ่าน/ไม่ผ่านตอนนี้ · เดดไลน์ที่ใกล้ปิด ·
git สะอาดไหม · และ **แผนที่ว่าไฟล์ .md ไหน generate มาจาก YAML ไหนด้วยสคริปต์ตัวไหน**

ทำแบบนี้แทนการไล่ `ls`/`grep`/อ่านสคริปต์ทีละตัว — ได้ภาพครบกว่าและถูกกว่ามาก
...
### เช็ค repo GitHub — คนละเรื่องกับ linkcheck

```bash
./scripts/ghcheck.sh              # รายงาน
./scripts/ghcheck.sh --self-test  # 6 เคส ต้องผ่านครบ
```

`linkcheck.sh` อ่านแค่ HTTP code จึงมองไม่เห็นสองอย่างนี้ — **repo ที่ย้าย org** (301 → 200
เหมือนปกติทุกประการ) และ **repo ที่ archived** (ยังคืน 200 อยู่ดี) ทั้งคู่คือ "`status: ok`
แต่ของไม่เหมือนเดิม" ที่เตือนไว้ด้านล่าง ตัวนี้จับได้อัตโนมัติ 2 ใน 4 แบบ

**ทำไม 301 ถึงเป็นปัญหาทั้งที่ยังเข้าได้** — GitHub เลิก redirect ทันทีที่มีใครสร้าง repo
ชื่อเดิมขึ้นที่ path เก่า ลิงก์เราจะชี้ของคนอื่น **โดยยังคืน 200 เหมือนเดิม** เจอตอนรันครั้งแรก
8 ส.ค. 2026 สองตัว: `solana-developers/program-examples` → `solana-foundation/` และ
**`solana-foundation/anchor` → `otter-sec/anchor`** (ตัวหลังแปลว่าจะส่ง PR เข้า Anchor
ต้องคุยกับ OtterSec ไม่ใช่ Foundation — `entity-check.sh` จับต่อทันทีว่า evidence ในกราฟยังชี้ path เก่า)

**ไม่แก้ไฟล์ให้เอง โดยตั้งใจ** การย้าย org เป็นการตัดสินใจของคน และเจอเคสที่ขัดกันจริง —
`anchor-lang.com` ยังลิงก์ path เดิมอยู่ ส่วน crates.io ชี้ otter-sec แล้ว

**บทเรียนที่ได้ระหว่างเขียน: สัญญาณเตือนที่ไม่มีวันดับ คือสัญญาณที่คนเลิกอ่าน** — รอบแรก
มันเตือน `developer-content` ซ้ำทั้งที่ทำ `deprecate.sh` ไปแล้ว ถ้าปล่อยไว้อีกสองเดือน
คนจะชินกับสีแดงแล้วมองข้ามของจริง จึงเพิ่มให้เงียบเมื่อมีฟิลด์ `deprecated` แล้ว
พร้อมเคสที่ 6 ใน `--self-test` ที่พิสูจน์ว่ามันเงียบจริง
...
- **`norm()` ใช้ `printf '%s'` ไม่มีท้ายบรรทัด** — เรียกทีละครั้งไม่มีปัญหา แต่ถ้าเอามาวนสร้างไฟล์
  URL ทุกอันจะต่อกันเป็นบรรทัดเดียวแล้ว `grep -x` ไม่เจออะไรเลย ให้ห่อเป็น `printf '%s\n' "$(norm "$u")"`
